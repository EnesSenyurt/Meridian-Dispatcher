...
async def test_mongodb_adapter_execution():
    with patch("app.database.AsyncIOMotorClient") as mock_motor:
        ...
        # Method mocking inside collection requires properly mapping what execute does
        # col = self.db[collection], getattr(col, operation)
        # We can mock this using patch directly on the adapter method, or simpler:
        # We are actually testing 'execute' which gets the method and runs it.
        ...
        adapter = MongoDBAdapter(url="mongodb://fake", db_name="test_db")
        result = await adapter.execute("users", "insert_one", {"name": "test"})
        
        assert result == {"inserted_id": "123"}
        mock_operation.assert_called_once_with({"name": "test"})
